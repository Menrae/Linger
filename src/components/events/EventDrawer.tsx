...
function ExternalEventContent({
  ...
  return (
    <div className="px-4 pb-6 pt-1 space-y-3">
      ...
      <h2 className="text-white text-lg font-semibold leading-tight">{event.title}</h2>

      <div className="flex items-start gap-2 text-sm text-gray-300">
        <svg className="w-4 h-4 flex-shrink-0 mt-0.5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
      ...
      {event.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-1">
          {event.tags.map((tag) => <TagPill key={tag} tag={tag} />)}
        </div>
      )}

      {event.description && (
        <p className="text-gray-300 text-sm leading-relaxed pt-1">{event.description}</p>
      )}
      ...
      <div className="flex gap-2 pt-3">
        {event.url && (
          <a
            ...
            className="flex-1 bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium py-2.5 rounded-xl transition-colors duration-150 text-center"
          >
            View on Eventbrite →
          </a>
        )}
...
export function EventDrawer() {
  ...
  const selectedExternalEvent = useMapStore((s) => s.selectedExternalEvent);
  const setSelectedEvent = useMapStore((s) => s.setSelectedEvent);
  const user = useAuthStore((s) => s.user);

  // Skip Supabase fetch for external events — they aren't in our DB
  const { state, retry, rsvp, cancelRsvp } = useEventDetail(
    selectedExternalEvent ? null : selectedEventId,
    user?.id ?? null,
  );
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {selectedExternalEvent ? (
            <ExternalEventContent event={selectedExternalEvent} copied={copied} onShare={() => void handleShare()} />
          ) : (
            <>
